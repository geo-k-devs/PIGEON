...
dataset = pd.read_csv("./DATASET_OR_inland.csv", sep=';')

dataset_inland = dataset

X = dataset_inland.values[:, 2:12]
y = dataset_inland.values[:, 12:13]
X.shape, y.shape
...
model_chl_inland = model_nn

hist = model_chl_inland.fit(X_train_scaled,
                            y_train_scaled,
                            batch_size=5,
                            epochs=1000,
                            validation_data=(X_val_scaled, y_val_scaled),
...
predict_train = model_chl_inland.predict(X_train_scaled)
predict_val = model_chl_inland.predict(X_val_scaled)
predict_test = model_chl_inland.predict(X_test_scaled)
...
model_chl_inland.save("./nn_OR_inland.h5")
...
plt.plot([0,10],[0,10])
plt.scatter(y_test, predict_test_inverse, c='r', s=10)
plt.scatter(y_train, predict_train_inverse, c='r', s=10)
plt.scatter(y_val, predict_val_inverse, c='r', s=10)
plt.xlabel("Chl-a Concentration (mg/m³)", size = 10,)
plt.ylabel("Estimated OR Chl-a Concentration (mg/m³)", size = 10)
plt.title('Estimation OR Inland')
plt.xscale("linear")
plt.yscale("linear")
plt.axis('square')
plt.tight_layout()
